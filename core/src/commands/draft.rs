...
/// relative to the project root.  The key is derived from the file's path
/// relative to the project directory, with `/` replaced by `__` so that
/// files in different subdirectories never collide.
fn draft_path(project_dir: &str, file_path: &str) -> Result<std::path::PathBuf, AppError> {
    let project = Path::new(project_dir);
    let file = Path::new(file_path);

    // Try to strip the project prefix to get a relative key.
    // If the file is outside the project (e.g. scratch files), fall back to
    // the file name only.
    let key = file
    ...
    let draft_dir = project.join(".novelist").join("drafts");
    Ok(draft_dir.join(format!("{}.draft.md", key)))
}
...
mod tests {
    ...
    fn test_draft_path_nested_file() {
        // Nested files use "__" separator to preserve uniqueness
        let p = draft_path("/project", "/project/chapters/ch1.md").unwrap();
        assert_eq!(
            p,
            Path::new("/project/.novelist/drafts/chapters__ch1.md.draft.md")
        );
    }
