...
>
  <span data-testid="status-word-count">{t('status.words', { count: wordCount })}</span>
  {#if goalPercent > 0}
  ...
  <span class="ml-auto flex items-center gap-2">
    {#if activeFileName}
      <span data-testid="status-file-name">{activeFileName}</span>
      {#if fileSize}
        <span class="opacity-50">·</span>
        <span>{fileSize}</span>
      {/if}
      <span class="opacity-50">·</span>
    {/if}
    <span data-testid="status-cursor-pos">{t('status.lineCol', { line: cursorLine, col: cursorCol })}</span>
  </span>
</div>
